    ...
    public class MaterialTabSelector : Control, IMaterialControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            foreach (TabPage tabPage in _baseTabControl.TabPages)
            {
                ...
                g.DrawString(tabPage.Text.ToUpper(), SkinManager.ROBOTO_MEDIUM_12, textBrush, _tabRects[currentTabIndex], new StringFormat { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
                textBrush.Dispose();
            }
        ...
        private void UpdateTabRects()
        {
            ...
            using (var b = new Bitmap(1, 1))
            {
                ...
                {
                    _tabRects.Add(new Rectangle(SkinManager.FORM_PADDING, 0, TAB_HEADER_PADDING * 2 + (int)g.MeasureString(_baseTabControl.TabPages[0].Text, SkinManager.ROBOTO_MEDIUM_12).Width, Height));
                    for (int i = 1; i < _baseTabControl.TabPages.Count; i++)
                    {
                        _tabRects.Add(new Rectangle(_tabRects[i - 1].Right, 0, TAB_HEADER_PADDING * 2 + (int)g.MeasureString(_baseTabControl.TabPages[i].Text, SkinManager.ROBOTO_MEDIUM_12).Width, Height));
                    }
                }
            }
        }
